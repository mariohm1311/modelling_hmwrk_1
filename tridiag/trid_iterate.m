function [x,k] = trid_iterate(method,A,b,x,parameters)

    %% Process method selection
    switch method
        case 'jacobi'
            step = @(A,b,x)trid_jacobi(A,b,x);
        case 'gaussseidel'
            step = @(A,b,x)trid_gaussseidel(A,b,x);        
        case 'sor'
            step = @(A,b,x)trid_sor(A,b,x,parameters.omega);
        otherwise
